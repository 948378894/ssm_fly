...
layui.define(['laypage', 'fly', 'element'], function(exports){
  ...
  if($('.upload-img')[0]){
    layui.use('upload', function(upload){  
      ...
      var avatar = $('.avatar');  //这行不能自己手写，复制上面那行，然后修改就可以了
      layui.upload({
        elem: '.upload-img input'
        ,method: 'post'
        ,url: '/yklgbbs/upload.do'  //这里是图片上传到的路径
        							//不是说服务器路径会经常变化吗？
        ,before: function(){
          avatarAdd.find('.loading').show();
        }
        ,success: function(res){  //想当于原生xhr.status===200{ status:0,src: path,msg:"成功"}
          if(res.status == 0){
        	  avatarAdd.find('img').attr('src',res.src); //用于实时显示上传的头像
        	  avatar.find('img').attr('src',res.src);
